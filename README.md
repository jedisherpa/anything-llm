...
- Run `corepack enable` once on a fresh machine so the repo uses the pinned Yarn version.
- `corepack yarn setup` To fill in the required `.env` files you'll need in each of the application sections (from root of repo).
  - Go fill those out before proceeding. Ensure `server/.env.development` is filled or else things won't work right.
- `corepack yarn dev:server` To boot the server locally (from root of repo).
- `corepack yarn dev:frontend` To boot the frontend locally (from root of repo).
- `corepack yarn dev:collector` To then run the document collector (from root of repo).

### Database modes

- Default local and desktop development stays on SQLite.
- PostgreSQL prep is available for scalable server deployments without forcing a desktop runtime cutover yet:
  - `corepack yarn prisma:prepare:postgres`
  - `corepack yarn prisma:generate:postgres`
  - `corepack yarn prisma:migrate:postgres`
