...
const Workspace = {
  ...
  multiplexStream: async function ({
    ...
    if (!!threadSlug)
      return this.threads.streamChat(
        ...
        chatHandler,
        attachments,
        { executionMode, trustedSessionId, executionContext }
      );
  ...
  executeSessionAction: async function (
    { slug },
    { trustedSessionId, action, pendingActionId = null, reason = null }
  ) {
    return await fetch(`${API_BASE}/workspace/${slug}/execute-session/action`, {
      method: "POST",
      body: JSON.stringify({
        trustedSessionId,
        action,
  ...
  executionStatus: async function ({ slug }, { trustedSessionId = null } = {}) {
    const params = new URLSearchParams();
    if (trustedSessionId) {
      params.set("trustedSessionId", trustedSessionId);
    }

    const query = params.toString();
    return await fetch(
      `${API_BASE}/workspace/${slug}/execute-session/status${query ? `?${query}` : ""}`,
      {
        method: "GET",
        headers: baseHeaders(),
      }
  ...
  updateExecutionConfig: async function ({ slug }, { demoModeEnabled }) {
    return await fetch(`${API_BASE}/workspace/${slug}/execute-session/config`, {
      method: "POST",
      body: JSON.stringify({
        demoModeEnabled,
      }),
